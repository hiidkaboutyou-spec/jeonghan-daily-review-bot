...
class TelegramBot:
    ...
    def send_message(
        ...
        *,
        ...
        data: dict[str, Any] = {
            "chat_id": chat_id or self.review_chat_id,
            "text": text[:4096],
    ...
    def _send_single_media(self, item: PreparedMedia) -> dict[str, Any]:
        ...
        with item.path.open("rb") as handle:
            ...
            data = {"chat_id": self.review_chat_id}
            if item.kind == "video":
    ...
    def _send_media_group(self, items: list[PreparedMedia]) -> list[dict[str, Any]]:
        ...
        try:
            for index, item in enumerate(items):
                ...
                descriptor = {"type": "photo" if item.kind == "photo" else "video", "media": f"attach://{key}"}
                if item.kind == "video":
    ...
    def is_admin_message(self, message: dict[str, Any]) -> bool:
        ...
        return int(sender.get("id", 0)) == self.admin_user_id and int(chat.get("id", 0)) == self.review_chat_id
...
def main_keyboard() -> dict[str, Any]:
    return inline_keyboard(
        [
            [("🕑 همهٔ دو ساعت اخیر", "cmd:recent2h")],
            [("🔎 جست‌وجوی آرشیو", "cmd:search"), ("🗂 منبع ۲۴ساعته", "cmd:sources")],
            [("📋 وضعیت", "cmd:status"), ("❔ راهنما", "cmd:help")],
        ]
    )
