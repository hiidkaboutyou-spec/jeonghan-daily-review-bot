...
class ChannelStyleReviewApplication(ReminderReviewApplication):
    """Production private-review application using ChannelStyleCaptionWriter first.

    Legacy CaptionWriter is retained only as a safe fallback/compatibility writer.
    """
    ...
    def __init__(self, settings):
        ...
        if not ok:
            ...
            logger.error("Channel style translation disabled; safe legacy writer retained: %s", reason)
            return
        ...
        try:
            ...
            logger.error(
                "Channel style writer initialization failed; safe legacy writer retained: %s",
                type(exc).__name__,
            )
            return
        ...
        logger.info(
            "Channel style translation v1 active as PRIMARY production writer with %s historical examples.",
            indexed,
        )
