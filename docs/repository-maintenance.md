...
PR #97/#98 and the later retirement completed the human-quality-gate migration; PR #101–#104 completed quality-repair migration/retirement; PR #105 completed the X-recovery coverage precursor. PR #106 established the migration sequence from fresh LibCST/Grimp evidence. The active migration now moves the unchanged base implementation to `app.x_resumable_recovery_runtime` under a same-module-object compatibility shim; `app`, integrity hardening, completeness provider-proof, and all audited direct test/patch references bind to the canonical base path. Persisted key `x_retrieval_checkpoints`, checkpoint IDs/schema normalization, source-scoped retry semantics and cursor advancement contracts remain unchanged. Keep exactly one active shim, require independent production proof plus a fresh final reference audit before retirement, and only then begin the separate `app.phase3_recovery_hardening` → `app.x_recovery_integrity_runtime` move.
