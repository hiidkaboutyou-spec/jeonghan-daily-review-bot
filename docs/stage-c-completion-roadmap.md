...
- `app.phase2_correlation_stability` → `app.lifecycle_correlation_runtime` — **current focused retirement step; post-merge proof required before C starts**.
- `app.live_recovery_hardening` → `app.x_degraded_recovery_runtime` — **final registered shim; retire last** because it owns mutable install/classification state and the degraded-X production recovery path.

Each retirement remains a separate focused PR. Never combine independent shim removals into one change.
...
## A. Lifecycle outcome visibility — complete

`app.phase2_final_visibility` was retired in PR #89 only after its compatibility references were isolated to tests/registry evidence. The canonical runtime remained between `phase2_runtime_compat` and `lifecycle_correlation_runtime`. Main run #4092 then completed restore, live-provider checks, a full monitor pass, checkpoint, encrypted backup/outcome upload, and state/database persistence without the shim.

## B. Lifecycle correlation — current

Retire `app.phase2_correlation_stability` second. Preserve `app.lifecycle_correlation_runtime` in the same import-order position and retain the stable first-event/translation-job identity behavior. The pre-removal plan from PR #89's Maintenance artifact shows no production importer; remaining references are compatibility-test/registry evidence. Do not advance to C until this retirement independently passes full CI and a real post-merge production run.

## C. Degraded X recovery — final registered shim

Retire `app.live_recovery_hardening` last. This is the highest-sensitivity registered shim because `app.x_degraded_recovery_runtime` owns mutable install flags and wraps the final scheduled scan. Remove only after A and B have independently passed production and after a fresh exact recovery reference audit confirms the legacy path is compatibility-only.
...
Record that decision in `docs/repository-maintenance.md`. The registered-shim subphase is complete when `config/module_migrations.json` has no `compatibility-shim` records, Maintenance generates no plans for retired migrations, and the final zero-shim production validation is green.
