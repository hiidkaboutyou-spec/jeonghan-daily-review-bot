...
The next Stage C candidate is `app.channel_part4_qualityfix`. Do not rename it opportunistically. Begin with a fresh planning-first LibCST/import-order audit after the human-quality shim retirement is independently production-proven, then preserve its mutation of the canonical human-quality-gate fingerprint/verifier state through any later semantic migration.
