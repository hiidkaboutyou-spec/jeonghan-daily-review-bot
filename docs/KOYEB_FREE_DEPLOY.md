...
## Required Koyeb secrets/environment variables

Copy the same runtime values already used by the GitHub production bot:
...
3. On the next scheduled GitHub run, `getWebhookInfo` reports the live webhook. GitHub stops using `getUpdates` and instead sends one authenticated `/maintenance` request to the webhook service.
...
- The Koyeb web service runs one Uvicorn worker so private state is processed serially.
