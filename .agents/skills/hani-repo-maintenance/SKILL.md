...
PR #97/#98 and the later focused retirement completed the human-quality-gate migration; PR #101–#104 completed quality-repair migration/retirement; PR #105 completed X-recovery coverage; PR #106 planned the recovery boundaries; PR #108 migrated the base implementation to `app.x_resumable_recovery_runtime`. Main run #4205 independently production-proved that move, and Maintenance #125 found only two migration-test strings plus zero direct/downstream importers for `app.phase3_recovery`. The base shim is therefore retired and must stay absent. Preserve `x_retrieval_checkpoints`, CHECKPOINT_VERSION, checkpoint identity/normalization, retries and cursor semantics. The next Stage C gate is fresh planning for `app.phase3_recovery_hardening` → `app.x_recovery_integrity_runtime`; inspect that new plan before changing the integrity implementation.
