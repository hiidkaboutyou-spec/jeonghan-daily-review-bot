...
PR #97/#98 and the later focused retirement completed the human-quality-gate migration. PR #101 fresh-planned `app.channel_part4_qualityfix` at 99.1% coverage / 129 contexts with four structural LibCST references and only `app/__init__.py` order-sensitive. PR #102 then production-proved the benchmark-freshness precursor. The active migration is `app.channel_part4_qualityfix` → `app.channel_quality_repair_runtime`: keep the old path as a same-module-object shim until a later focused retirement, keep the benchmark fingerprint on the canonical implementation, and preserve exact install order plus mutation of the canonical human-gate fingerprint/verifier state.
