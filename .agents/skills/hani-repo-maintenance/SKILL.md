...
Stage C compatibility migrations are authoritative in `config/module_migrations.json`. All currently registered legacy paths, including `app.channel_part4_humanfix`, are retired and must stay absent. Do not recreate a retired path merely because a future caller or test still uses an old historical name; migrate the caller to the canonical module instead.

PR #97/#98 and the later focused retirement completed the human-quality-gate migration; PR #101–#104 completed quality-repair migration/retirement; PR #105 completed the X-recovery coverage precursor. PR #106 fresh-planned both recovery boundaries: base recovery has 56 references (11 structural / 45 dynamic-manual / one order-sensitive) and integrity hardening has six (two structural / four dynamic-manual / one order-sensitive), with zero parse errors. `phase3_recovery_hardening` mutates `_update_dicts`, `_sanitize_checkpoint`, and `_lookup_user` on the base module object, while `completeness_provider_proof` replaces `_provider_page` on that same object. The active migration now moves `app.phase3_recovery` → `app.x_resumable_recovery_runtime` with the implementation byte-for-byte unchanged and the old path as a same-module-object shim. Production callers and audited test patch strings must target the canonical base module; Maintenance focused coverage must measure the canonical file. Preserve `x_retrieval_checkpoints`, CHECKPOINT_VERSION, checkpoint identity/normalization, retries and cursor semantics. Obtain full CI, independent real-main production proof, and a fresh final reference audit before retiring the base shim. Only after base retirement/proof may `app.phase3_recovery_hardening` → `app.x_recovery_integrity_runtime` begin. Never combine the two implementation moves.
