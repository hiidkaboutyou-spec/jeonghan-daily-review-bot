...
PR #97/#98 and the later focused retirement completed the human-quality-gate migration; PR #101–#104 completed quality-repair migration/retirement. PR #105 completed the recovery-family coverage precursor with no production-code change: `app.phase3_recovery` measures 76.6% / 129 contexts and `app.phase3_recovery_hardening` 99.2% / 33 contexts, with direct checkpoint/state/retry/fallback/integrity regression coverage. Do not chase the remaining gross base percentage by invoking superseded implementations: startup intentionally replaces its sanitizer and profile lookup through `phase3_recovery_hardening` and its raw provider-page implementation through `completeness_provider_proof`. The next gate is planning-first LibCST/reference/import-order evidence for both `app.x_resumable_recovery_runtime` and `app.x_recovery_integrity_runtime`; design their sequence together before either move.
