...
Stage C compatibility migrations are authoritative in `config/module_migrations.json`. All registered legacy paths are retired and must stay absent, including `app.phase3_recovery`, `app.phase3_recovery_hardening`, `app.channel_part4_humanfix`, and `app.channel_part4_qualityfix`. Do not recreate a retired path merely because a future caller or test still uses an old historical name; migrate the caller to the canonical module instead.

PR #97/#98 and the later focused retirement completed the human-quality-gate migration; PR #101–#104 completed quality-repair migration/retirement; PR #105 completed X-recovery coverage; PR #106 planned the recovery boundaries; PR #108/#110 migrated and retired the base implementation as `app.x_resumable_recovery_runtime`; PR #111 migrated the integrity layer to `app.x_recovery_integrity_runtime`; PR #112 retires the final registered compatibility path `app.phase3_recovery_hardening` after fresh evidence showed zero structural/import-order-sensitive references and zero direct/downstream importers. Preserve `x_retrieval_checkpoints`, CHECKPOINT_VERSION, checkpoint identity/normalization, retries, provider fallback, source authorization and cursor semantics. After PR #112 merges, require independent real-main production proof before any additional Stage C structural migration. Then generate a fresh repository inventory and module-family evidence report; do not choose the next family from stale closure-audit ordering. The four retain-by-design historical modules remain outside the rename queue unless their architecture materially changes.
