...
Stage C compatibility migrations are authoritative in `config/module_migrations.json`. Previously retired legacy paths, including `app.channel_part4_humanfix` and `app.channel_part4_qualityfix`, must stay absent. The only active compatibility shim is `app.phase3_recovery` → `app.x_resumable_recovery_runtime`; do not recreate any retired path, and do not create a second active shim while this migration remains open.

PR #97/#98 and the later focused retirement completed the human-quality-gate migration; PR #101–#104 completed quality-repair migration/retirement; PR #105 completed the X-recovery coverage precursor; PR #106 planned both recovery boundaries. The base migration `app.phase3_recovery` → `app.x_resumable_recovery_runtime` is now the single active Stage C migration. The canonical file owns the unchanged former implementation; `app.phase3_recovery` must remain a same-module-object alias until independent production proof and a later focused retirement. Production startup, `phase3_recovery_hardening`, and `completeness_provider_proof` must patch/import the canonical object. Preserve `x_retrieval_checkpoints`, CHECKPOINT_VERSION, checkpoint IDs/schema normalization, bounded retries/fallbacks, source-scoped cursor semantics, and exact import order. Do not start `app.phase3_recovery_hardening` → `app.x_recovery_integrity_runtime` while the base shim is active; first production-prove and retire the base shim.
