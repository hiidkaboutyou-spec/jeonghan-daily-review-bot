...
PR #97/#98 and the later focused retirement completed the human-quality-gate migration. PR #101/#102 prepared the quality-repair boundary and benchmark freshness, PR #103 migrated the implementation byte-for-byte to `app.channel_quality_repair_runtime`, and the later focused retirement removed `app.channel_part4_qualityfix` only after real-main production proof and a fresh audit showing zero importers plus only compatibility-registry strings. Both historical paths are retired and must stay absent. The next Stage C candidate family is `app.phase3_recovery` + `app.phase3_recovery_hardening`; strengthen critical checkpoint/state/retry coverage before planning a semantic migration, and design base/integrity sequencing together without combining broad behavior changes.
