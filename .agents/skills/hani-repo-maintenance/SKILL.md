...
Stage C compatibility migrations are authoritative in `config/module_migrations.json`. All registered legacy paths are retired after the base recovery retirement and must stay absent, including `app.phase3_recovery`, `app.channel_part4_humanfix`, and `app.channel_part4_qualityfix`. Do not recreate a retired path merely because a future caller or test still uses an old historical name; migrate the caller to the canonical module instead.

PR #97/#98 and the later focused retirement completed the human-quality-gate migration; PR #101–#104 completed quality-repair migration/retirement; PR #105 completed X-recovery coverage; PR #106 planned the recovery boundaries; PR #108 migrated the base implementation to `app.x_resumable_recovery_runtime`. Main run #4205 independently production-proved that move, and Maintenance #125 found only two migration-test strings plus zero direct/downstream importers for `app.phase3_recovery`. The base shim is therefore retired and must stay absent. Preserve `x_retrieval_checkpoints`, CHECKPOINT_VERSION, checkpoint identity/normalization, retries and cursor semantics. PR #110 retired `app.phase3_recovery` and main Daily #4208 plus Maintenance #128, Security #136, CodeQL #97, Render #305, Fanfic #977, Workflow Safety #13 and Watchdog #3311 independently production-proved the retirement. Maintenance #127 then provided fresh integrity evidence: `app.phase3_recovery_hardening` → `app.x_recovery_integrity_runtime` has six references (two structural, four synthetic/manual fixture strings), one import-order-sensitive reference, zero parse errors, 99.2% coverage across 33 contexts, and only `app` as a runtime importer. This integrity migration is now the single active Stage C shim. The canonical implementation must remain byte-for-byte unchanged, patch only the canonical `app.x_resumable_recovery_runtime` object, preserve exact import order and all `x_retrieval_checkpoints`/retry/cursor semantics, then receive full CI + independent real-main proof + a fresh retirement audit before the shim can be deleted.
