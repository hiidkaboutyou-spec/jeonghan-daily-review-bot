...
"""Credential-safe GitHub Actions artifact transport for the Daily watchdog.

GitHub's artifact archive endpoint returns a short-lived redirect URL. Python's
ordinary Request headers are copied to redirected requests, so the authenticated
GitHub API request must not be allowed to forward repository credentials to the
signed download host.

This module is intentionally side-effect free on import. Production callers must
invoke :func:`install` before running the watchdog decision engine.
"""
...
except ModuleNotFoundError:  # direct execution/import from tools directory
...
def install() -> None:
    """Install the hardened artifact fetcher on the canonical watchdog client."""
