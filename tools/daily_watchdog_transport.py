...
"""Run the Daily watchdog with credential-safe artifact transport.

GitHub's Actions artifact download endpoint returns a short-lived redirect URL.
The authenticated GitHub API request is therefore handled separately from the
redirected signed-host request so repository credentials never cross origins.
"""
...
def install_transport() -> None:
    """Install the credential-safe outcome fetcher on the watchdog client."""
