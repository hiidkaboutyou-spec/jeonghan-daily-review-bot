...
class SecurityDiagnosticsConfigurationTests(unittest.TestCase):
    def test_security_workflow_is_pinned_and_secret_free(self) -> None:
        ...
        for sha in (CHECKOUT_SHA, SETUP_PYTHON_SHA, UPLOAD_ARTIFACT_SHA, DEPENDENCY_REVIEW_SHA):
            self.assertIn(sha, text)
        self.assertNotIn("secrets.", text)
        self.assertIn("fail-on-severity: high", text)
        self.assertIn("license-check: false", text)
    ...
    def test_manifest_records_scanners_as_non_runtime(self) -> None:
        ...
        expected = {
            ...
            "github-dependency-review": DEPENDENCY_REVIEW_SHA,
        }
